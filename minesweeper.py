...
class MinesweeperAI():
            ...
            for dj in range(-1, 2):
                if (di, dj) != (0, 0):
                    ...
                    if 0 <= ni < self.height and 0 <= nj < self.width:
                        if (ni, nj) not in self.moves_made and (ni, nj) not in self.safes and (ni, nj) not in self.mines:
                            neighbors.add((ni, nj))
    ...
    def update_knowledge(self):
        """
        Updates the AI's knowldge base, marking cells as safe or mines if possible.
        """
        made_changes = True
        while made_changes:
            made_changes = False
